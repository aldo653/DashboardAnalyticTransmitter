...
export default function CardCooling({ data }) {
  ...
  const chartData = useMemo(() => {
    ...
    safeData.forEach((row) => {
      ...
      const rawValue = row["Cooling Temperature (°C)"];
      if (!rawValue) return;
    ...
    return Object.entries(grouped)
      .map(([date, { total, count, dateObj }]) => ({
        tanggal: date,
        CoolingTemp: count > 0 ? Number((total / count).toFixed(2)) : 0,
        _dateObj: dateObj
      }))
      ...
      <div className="card w-100" style={{ height: "420px" }}>
        <div className="card-body">
          <div className="d-flex justify-content-between">
            <div>
              <h6 className="fw-semibold mb-3"><i className="ti ti-snowflake text-primary me-2 fw-bold"></i>Rata-Rata Cooling Temperature (°C)</h6>
            </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={chartData}>
              <defs>
                <linearGradient id="colorCoolingTemp" x1="0" y1="0" x2="0" y2="1">
