...
export default function CardtrendlistrikallReg({ data }) {
    ...
    const uniqueRegions = useMemo(() => {
        const regions = safeData
            .map((row) => row["Satuan Transmisi Daerah"])
            .filter((v) => v && v.trim() !== "");
        return [...new Set(regions)];
    }, [safeData]);

    // Set default region ke "Prabumulih" jika tersedia
    useEffect(() => {
        if (uniqueRegions.includes("Prabumulih")) {
            setSelectedRegion("Prabumulih");
        } else {
            setSelectedRegion(""); // fallback: semua data
        }
    }, [uniqueRegions]);
    ...
    const chartData = useMemo(() => {
        ...
        safeData.forEach((row) => {
            ...
            if (selectedRegion && row["Satuan Transmisi Daerah"] !== selectedRegion)
                return;
            ...
            <div className="card w-100" style={{ height: "480px" }}>
                <div className="card-body">
                    ...
                    <h6 className="fw-semibold mb-3 text-center">
                        ...
                        {selectedType === "EI"
                            ? "Electrosys Input (KWatt)"
                            : "Electrosys Output (KWatt)"} {selectedRegion}
                    </h6>
