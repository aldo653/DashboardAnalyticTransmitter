...
export default function SpeedometerTvri({ data }) {
  ...
  const [selectedChannel, setSelectedChannel] = useState("TVRI SUMSEL");
  const [width, setWidth] = useState(340);
  const containerRef = useRef(null);

  // Ubah ukuran gauge otomatis mengikuti lebar card
  useEffect(() => {
    const handleResize = () => {
  ...
  // Set tanggal default ke data terakhir (pakai waktu lokal)
  useEffect(() => {
